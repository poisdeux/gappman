...
static void parseMessage(struct proceslist** procs, gchar *msg)
{
...
struct proceslist* getProceslistFromGappman(int portno, const char* hostname)
{
  ...
  GIOChannel* gio = NULL;
  GError *gerror = NULL;
	struct proceslist* startedprocs = NULL;
  ...
  if (server == NULL)
  {
    fprintf(stderr,"Error: could not find host %s", hostname);
    exit(0);
  }
  ...
  while( g_io_channel_read_line( gio, &msg, &len, NULL,  &gerror) != G_IO_STATUS_EOF )
  {
    fprintf(stdout, "MESSAGE RECEIVED: %s\n", msg);
		parseMessage(&startedprocs, msg);
  }

  status = g_io_channel_shutdown( gio, TRUE, &gerror);
  if ( status == G_IO_STATUS_ERROR )
  {
     g_error ("test-listener (handlemessage): %s\n", gerror->message);
  }
