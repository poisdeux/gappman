...
static gboolean egg_clock_face_expose(GtkWidget * clock,
...
static void egg_clock_face_class_init(EggClockFaceClass * class)
{
	...
	egg_clock_face_signals[TIME_CHANGED] = g_signal_new("time-changed",
														G_OBJECT_CLASS_TYPE
														(obj_class),
														G_SIGNAL_RUN_FIRST,
														G_STRUCT_OFFSET
...
static void draw(GtkWidget * clock, cairo_t * cr)
{
	...
	/* hour hand: the hour hand is rotated 30 degrees (pi/6 r) per hour + 1/2
	   a degree (pi/360 r) per minute */
	cairo_save(cr);
...
static void egg_clock_face_redraw_canvas(EggClockFace * clock)
{
	GtkWidget *widget;
	GdkRegion *region;
...
static emit_time_changed_signal(EggClockFace * clock, int x, int y)
{
	...
	/* phi is a bearing from north clockwise, use the same geometry as we did
	   to position the minute hand originally */
	phi = atan2(x, -y);
	if (phi < 0)
		phi += M_PI * 2;
	...
	g_signal_emit(clock,
				  egg_clock_face_signals[TIME_CHANGED], 0, hour, minute);
}
...
egg_clock_face_motion_notify(GtkWidget * clock, GdkEventMotion * event)
{
	...
	if (priv->dragging)
	{
		emit_time_changed_signal(EGG_CLOCK_FACE(clock), event->x, event->y);
	}
}
