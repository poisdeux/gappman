...
typedef int (* GM_MODULE_INIT) (void); ///< Function to initialize the module
typedef GtkWidget* (* GM_MODULE_WIDGET) (void); ///< Function returning the widget the module use. If any/
typedef void (* GM_MODULE_SET_ICON_SIZE) (int width, int height); ///< Function to set the width and height of the image used by the module, if any.
typedef void (* GM_MODULE_SET_CONFFILE) (const char* filename); ///< Function for gappman to provide the location of its configuration file to the module.
typedef void* (* GM_MODULE_START) (void); ///< Function to start the module. Should be called after GM_MODULE_INIT.
typedef int (* GM_MODULE_STOP) (void); ///< Function to stop the module. This should release all memory occupied by the module. Gappman does no garbage collection.
...
struct menu_element
{
    ...
    struct length *menu_width; ///< holds the width of the menu this element is a part of. Note that all elements in the same menu should point to the same length struct for menu_width.
    struct length *menu_height; ///< holds the height of the menu this elements is a part of. Note that all elements in the same menu should point to the same length struct for menu_width.
    int app_width; ///< screen resolution width that should be used when the application of this menu_element is started.
    int app_height; ///< screen resolution height that should be used when the application of this menu_element is started.
